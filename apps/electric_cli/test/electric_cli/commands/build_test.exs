defmodule ElectricCli.Commands.BuildTest do
  ...
  describe "electric build" do
    ...
    test "succeeds", cxt do
      args = argv(cxt, [])
      assert {{:ok, output}, _} = run_cmd(args)
      assert output =~ "Built successfully"
    end

    test "builds the index.js", %{tmp_dir: root} = cxt do
    ...
    test "the index.js file is marked as a local build", %{tmp_dir: root} = cxt do
      args = argv(cxt, [])
      assert {{:ok, _output}, _} = run_cmd(args)
    ...
    test "default manifest unchanged without flags", %{tmp_dir: root} = cxt do
    ...
    test "manifest updated with satellite flag", %{tmp_dir: root} = cxt do
      args = argv(cxt, ["--satellite"])
      assert {{:ok, _output}, _} = run_cmd(args)
    ...
    test "manifest updated with postgres flag", %{tmp_dir: root} = cxt do
      args = argv(cxt, ["--postgres"])
      assert {{:ok, _output}, _} = run_cmd(args)
    ...
    test "can load index.js from @config alias", %{tmp_dir: root} = cxt do
      args = argv(cxt, [])
      assert {{:ok, _output}, _} = run_cmd(args)
    ...
    test "can load index.js from @app alias", %{tmp_dir: root} = cxt do
      args = argv(cxt, [])
      assert {{:ok, _output}, _} = run_cmd(args)
  ...
  describe "electric build --env ENV" do
    ...
    test "target env must exist", cxt do
      args = argv(cxt, ["--env", "missing"])
      assert {{:error, output}, _} = run_cmd(args)
      assert output =~ "not found"
    end

    test "builds for the target env", %{tmp_dir: root} = cxt do
