defmodule Electric.Migrations.Parse do
  ...
  def ast_from_ordered_migrations(migrations) do
    ...
    sql_errors = Enum.flat_map(migrations, fn migration ->
    ...
    if length(sql_errors) > 0 do
      ...
      infos =
        for [type, name, tbl_name, rootpage, sql] <- info do
          ...
          # foreign keys
          {:ok, foreign_statement} = Exqlite.Sqlite3.prepare(conn, "PRAGMA foreign_key_list(#{tbl_name});")
          ...
          %{
            :table_name => tbl_name,
            :table_info => %{type: type, name: name, tbl_name: tbl_name, rootpage: rootpage, sql: sql},
      ...
      validation_fails = for info <- infos, length(info.validation_fails) > 0 do
  ...
  def check_sql(table_name, sql) do
    ...
    validation_fails = if !String.contains?(lower, "strict") do
      ["The table #{table_name} is not STRICT. Add the STRICT option at the end of the create table statement" | validation_fails]
    else
      validation_fails
    end

    validation_fails = if !String.contains?(lower, "without rowid") do
      ["The table #{table_name} is not WITHOUT ROWID. Add the WITHOUT ROWID option at the end of the create table statement and make sure you also specify a primary key" | validation_fails]
    else
      validation_fails
    end
  end
      ...
      for [type, name, tbl_name, rootpage, sql] <- info, into: %{} do
        ...
        index_infos =
          for [seq, name, unique, origin, partial] <- indexes, into: %{}  do
            ...
            {:ok, col_info_statement} = Exqlite.Sqlite3.prepare(conn, "PRAGMA index_xinfo(#{name});")
            index_columns = Enum.reverse(get_rows_while(conn, col_info_statement, []))

            index_column_infos = for [seqno, cid, name, desc, coll, key] <- index_columns do
              %{
                :seqno => seqno,
                :cid => cid,
                :name => name,
  ...
  defp is_unique(column_name, indexes) do
    case indexes do
      nil ->
        false
      _ ->
        matching_unique_indexes = for {seq, info} <- indexes do
        case info.origin do
          "u" ->
            cols = for key_column <- info.columns do
  ...
  defp is_primary_desc(column_name, indexes) do
    case indexes do
      nil ->
        false
      _ ->
        matching_desc_indexes = for {seq, info} <- indexes do
        case info.origin do
          "pk" ->
            cols = for key_column <- info.columns do
