defmodule Electric.Migrations.Generation do
  ...
  defp get_sql_for_infos(before_ast, after_ast, flavour) do
    ...
    statements = for change <- table_changes(before_ast, after_ast) do
      case change do
        {nil, table_after} ->
          ## https://www.sqlite.org/syntax/column-constraint.html
          ## %{cid: 0, dflt_value: nil, name: "id", notnull: 0, pk: 1, type: "INTEGER"}

          column_definitions = for {column_id, column_info} <- table_after.column_infos do
            "\n  " <> column_def_sql_from_info(column_info, flavour)
          end

          foreign_key_clauses = for foreign_key_info <- table_after.foreign_keys_info do
        ...
        ## add columns
        added_colums_lines = for {column_id, column_info} <- table_after.column_infos, !Map.has_key?(table_before.column_infos, column_id) do
          "ALTER TABLE #{table_after.namespace}.#{table_after.table_name} ADD COLUMN #{column_def_sql_from_info(column_info, flavour)};\n"
        end

        ## delete columns
        dropped_colums_lines = for {column_id, column_info} <- table_before.column_infos, !Map.has_key?(table_after.column_infos, column_id) do
          "ALTER TABLE #{table_before.namespace}.#{table_before.table_name} DROP COLUMN #{column_info.name};\n"
        end

        ## rename columns
        rename_colums_lines = for {column_id, column_info} <- table_after.column_infos, Map.has_key?(table_before.column_infos, column_id) && column_info.name != table_before.column_infos[column_id].name do
  ...
  defp table_changes(before_ast, after_ast) do
    if before_ast == nil do
      ...
      new_and_changed = for {table_name, table_info} <- after_ast, table_info != before_ast[table_name] do
         {before_ast[table_name], table_info}
      end
      dropped = for {table_name, table_info} <- before_ast, !Map.has_key?(after_ast, table_name) do
        {table_info, nil}
      end
  ...
  defp foreign_key_sql_from_info(key_info, flavour) do
    ...
    elements = if key_info.on_delete != "NO ACTION" do
     ["ON DELETE #{key_info.on_delete}" | elements]
    else
     elements
    end

    elements = if key_info.on_update != "NO ACTION" do
     ["ON UPDATE #{key_info.on_update}" | elements]
    else
     elements
    end

    elements = ["FOREIGN KEY(#{key_info.from}) REFERENCES #{key_info.table}(#{key_info.to})" | elements]
  ...
  defp column_def_sql_from_info(column_info, flavour) do
    ...
    elements = []
    elements = if column_info.dflt_value != nil do
      ["DEFAULT #{column_info.dflt_value}" | elements]
    else
      elements
    end
    elements = if column_info.unique do ["UNIQUE" | elements] else elements end
    elements = if column_info.notnull != 0 && column_info.pk == 0 do ["NOT NULL" | elements] else elements end
    elements = if column_info.pk != 0 do
      ["PRIMARY KEY#{ if column_info.pk_desc do " DESC" else "" end}" | elements]
    else
     elements
    end
